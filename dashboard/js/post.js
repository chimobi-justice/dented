...
const checkCompanyFields = () => {
    if (company_name.value.length && company_location.value.length && category.value.length && job_time.value.length && company_url.value.length && job_description.value.length > 0) {
...
category.addEventListener('keyup', checkCompanyFields);
